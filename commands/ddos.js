...
exports.run = async function (client, message, args) {
    ...
    setTimeout(function () {
        m.edit(`${loading} Target Information Acquiring... ${loading}`)
    }, 2000);
    setTimeout(function () {
        m.edit(`${loading} Target Information Acquired. ${loading}`)
    }, 5000);
    setTimeout(function () {
        m.edit(`${loading} Obtaining Target's IP Addresses... ${loading}`)
    }, 10000);
    setTimeout(function () {
        m.edit(`${loading} Obtained Target's IP Addresses. ${loading}`)
    }, 15000);
    setTimeout(function () {
        m.edit(`${loading} DDoSing Target From ${randm} Host Servers. ${loading}`)
    }, 20000);
    setTimeout(function () {
        m.edit(`Target ${ddosMember} has been DDoSed and all current infomation has been leaked`)
    }, 25000);
